...
def onpi(data, nStates, nActions):
    LData=data
...
def mrl(max_eps, env, max_steps):
    ...
    #build the raw data...
    data={s: {a: {next: [] for next in range(env.nS)} for a in range(env.nA)} for s in range(env.nS)}
    ...
    for t in range(max_eps):
        if t%20==0 and t>0:
            onpi_data=onpi(data, env.nS, env.nA)
            if t%300==0:
                print("Q is ", Q)
                check(env, onpi_data)
            Q, V, policy=ValueIteration(env, onpi_data) 
    ...
    for t in range(max_eps//20+1):
        ...
        reward_std[t]=np.std(quality[:t+1])
    print(quality)
