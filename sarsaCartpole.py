...
def SARSA(max_eps, env, nS, nA, max_steps, trained_Q=np.ones((1, 1)), trained_policy=np.ones(1), AlreadyTrained=False):
    ...
    for t in range(max_eps):
        ...
        for s in range(nS):
            policy[s]=eGreedy(epsilon(t, max_eps), Q, s, env)
...
def GreedyEval(env,nS, nA, max_steps, max_episodes, trained_Q=np.ones((1, 1)), trained_policy=np.ones(1), AlreadyTrained=False):
    Q=np.ones((nS, nA))
    policy=np.ones(nS, dtype=int)
    if AlreadyTrained:
        policy, Q=SARSA(max_episodes, env,nS, nA, max_steps, trained_Q, trained_policy, AlreadyTrained)
    else:
        print("NOT")
        policy, Q=SARSA(max_episodes, env,nS, nA, max_steps)
...
def Plot(env, nS, nA):
    ...
    for alpha in Alpha:
        ...
        if alpha-nRuns>0:
            policy, Q=GreedyEval(env, nS, nA, 2501000500, alpha-nRuns, Q,policy,True)
        else: # when
            policy, Q=GreedyEval(env,nS, nA, 2501000500, alpha) 
        nRuns=alpha
...
env,nS,nA,discCP,dname=prepCartPole()
Plot(env, nS, nA)